...
function ActionItem({
  ...
  const content = (
    <div className="flex items-start gap-3 rounded-xl border border-slate-200 p-3 bg-white hover:bg-slate-50 transition">
      ...
      {external && <ExternalLink className="h-4 w-4 text-slate-400" aria-hidden />}
    </div>
  );
...
export default function Navbar() {
  ...
  return (
    <header
      className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-gradient-to-b from-slate-900 to-slate-950 text-slate-200 shadow-lg"
      role="banner"
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 md:px-6">
        {/* Logo */}
        <a href="#home" className="group inline-flex items-center gap-2" aria-label="Inicio">
          <span className="relative inline-flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-600 to-teal-600 text-white shadow-md ring-1 ring-white/10">
            <svg
          ...
          <div className="leading-tight">
            ...
            <span className="block text-[11px] text-slate-400">Estado de México</span>
          </div>
        </a>
          ...
          {links.map((l) => {
            ...
            return (
              <li
                ...
                onMouseLeave={() => onLeave(() => setHoverKey((k) => (k === l.key ? null : k)))}
              >
                <button
                  type="button"
                  className={classNames(
                    'rounded-xl px-3 py-2 text-sm font-medium transition',
                    isOpen ? 'bg-white/10 text-white' : 'text-slate-200 hover:bg-white/5 hover:text-white'
                  )}
                  aria-haspopup="menu"
                ...
                <Flyout open={isOpen} bare={l.key === 'ubica'} center={l.key === 'ubica'}>
                  {l.key === 'ubica' ? (
        ...
        <div className="flex items-center gap-2">
          <a
            href="#"
            className="hidden items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2 text-sm font-semibold text-white shadow-md ring-1 ring-white/10 transition hover:opacity-95 active:opacity-100 md:inline-flex"
          >
            <Phone className="h-4 w-4" /> Agenda una llamada
          </a>
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-xl p-2 transition hover:bg-white/5 md:hidden"
